...
export default function AdminAnalyticsPage() {
  ...
  }, [user?.id, user?.role, timeRange, isLoading])

  const fetchAnalytics = async () => {
    try {
      setLoading(true)

      // Fetch total volume and transactions
      const { data: transactionsData, error: transactionsError } = await supabase
        .from('transactions')
        .select('amount, created_at')
        .gte('created_at', getTimeRangeStartDate(timeRange))
      ...
      if (usersError) {
        console.error("Error fetching total users for analytics:", usersError)
      }
      ...
      if (merchantsError) {
        console.error("Error fetching total merchants for analytics:", merchantsError)
      }

      // Placeholder for regional distribution and merchant types (requires more complex queries or dedicated tables)
      setAnalytics({
        ...
        regions: [
          { name: "North America", percentage: 45, volume: totalVolume * 0.45 },
          { name: "Europe", percentage: 30, volume: totalVolume * 0.30 },
          { name: "Asia Pacific", percentage: 20, volume: totalVolume * 0.20 },
          { name: "Other", percentage: 5, volume: totalVolume * 0.05 },
        ],
        merchantTypes: [
        ...
        transactionTrends: [
          { period: "Week 1", volume: totalVolume * 0.25, transactions: totalTransactions * 0.25 },
          { period: "Week 2", volume: totalVolume * 0.25, transactions: totalTransactions * 0.25 },
          { period: "Week 3", volume: totalVolume * 0.25, transactions: totalTransactions * 0.25 },
          { period: "Week 4", volume: totalVolume * 0.25, transactions: totalTransactions * 0.25 },
        ],
      })
